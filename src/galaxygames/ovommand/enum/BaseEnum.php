		...
		foreach ($aliases as $key => $alias) {
			if (is_string($alias)) {
				if (!isset($this->values[$key])) {
					throw new EnumException(ExceptionMessage::ENUM_ALIAS_UNKNOWN_KEY->translate(["aliasName" => $alias, "key" => $key]), EnumException::ENUM_ALIAS_UNKNOWN_KEY);
				}
				if (isset($this->showAliases[$alias]) || isset($this->hiddenAliases[$alias])) {
					throw new EnumException(ExceptionMessage::ENUM_ALIAS_REGISTERED->translate(["aliasName" => $alias]), EnumException::ENUM_ALIAS_REGISTERED);
				}
				$aliasesList[$alias] = $key;
			} elseif (is_array($alias)) {
				foreach ($alias as $a) {
					if (!isset($this->values[$key])) {
						throw new EnumException(ExceptionMessage::ENUM_ALIAS_UNKNOWN_KEY->translate(["aliasName" => $a, "key" => $key]), EnumException::ENUM_ALIAS_UNKNOWN_KEY);
					}
					if (!is_string($a)) {
						throw new EnumException(ExceptionMessage::ENUM_ALIAS_UNKNOWN_TYPE->translate(["key" => $key, "type" => gettype($a)]), EnumException::ENUM_ALIAS_UNKNOWN_TYPE);
					}
					if (isset($this->showAliases[$a]) || isset($this->hiddenAliases[$a])) {
						throw new EnumException(ExceptionMessage::ENUM_ALIAS_REGISTERED->translate(["aliasName" => $a]), EnumException::ENUM_ALIAS_REGISTERED);
					}
					$aliasesList[$a] = $key;
				}
			} else {
				throw new EnumException(ExceptionMessage::ENUM_ALIAS_UNKNOWN_TYPE->translate(["key" => $key, "type" => gettype($alias)]), EnumException::ENUM_ALIAS_UNKNOWN_TYPE);
			}
		}
	}
